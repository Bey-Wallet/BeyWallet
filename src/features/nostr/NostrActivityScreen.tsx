...
export default function NostrActivityModal() {
  ...
  const { data: nostrHistory = [], refetch: refetchHistory } = useQuery({
    ...
    queryFn: async () => {
      ...
      return entries.filter(
        (e: any) =>
          e.metadata?.nostr === true || e.metadata?.type === 'p2pk' || e.metadata?.p2pkPubkey,
      );
    },
    staleTime: 15_000,
    gcTime: 5 * 60_000,
  ...
  const claimed = useMemo(() => items.filter((i) => i.status === 'claimed'), [items]);
        ...
        {tabs.map((tab) => {
          ...
          return (
            <XStack
              ...
              cursor="pointer"
            >
              <Text fontSize="$3" fontWeight="700" color={isActive ? '$background' : '$gray10'}>
                {tab.label}
              </Text>
              {tab.count > 0 && (
                <View
                  ...
                  items="center"
                >
                  <Text fontSize={10} fontWeight="900" color={isActive ? '$color' : '$gray10'}>
                    {tab.count}
                  </Text>
                </View>
              )}
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <Theme inverse>
                  ...
                  >
                    <DetailSheetRow
                      pubkey={detailItem.pubkey}
                      storedUsername={detailItem.username}
...
function PendingTab({
  ...
  onClaim: (item: NostrInboxItem) => void;
  onDetail: (item: NostrInboxItem) => void;
  btcData: any;
            ...
            >
              <XStack items="center" gap="$3" flex={1}>
                ...
                <YStack flex={1}>
                  <ReceivedRowName
                    pubkey={item.senderPubkey}
                    storedUsername={item.senderUsername}
                  />
                  <Text fontSize="$1" color="$gray10">
