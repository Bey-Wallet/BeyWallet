...
import { YStack, XStack, Text, Button, View, ScrollView, Input, Separator } from 'tamagui';
import { ActivityIndicator, Alert } from 'react-native';
import { useRouter } from 'expo-router';
import { networkService } from '~/services/platform/networkService';
import { AtSign, CheckCircle, XCircle, Globe, Copy, Trash2, Edit3 } from '@tamagui/lucide-icons';
...
async function checkAvailability(username: string): Promise<'available' | 'taken' | 'error'> {
  try {
    const res = await fetch(`${API_BASE}/register?check=${encodeURIComponent(username)}`, {
...
export function NostrUsernameScreen() {
  ...
  const checkMessage = () => {
    ...
    if (!isValidFormat)
      return { text: 'Only lowercase letters, numbers, _ . - allowed', color: '$orange10' };
    ...
    if (checkState === 'taken')
      return { text: `✗ ${input}@${DOMAIN} is already taken`, color: '$red10' };
    if (checkState === 'error')
      return { text: 'Could not check availability — try again', color: '$orange10' };
    return null;
  };
          ...
          <YStack gap="$5">
            ...
            <YStack bg="$gray2" p="$4" rounded="$4" gap="$2">
              ...
              <Text fontSize="$2" color="$gray10" lineHeight={18}>
                A NIP-05 identifier (like an email address) lets other Nostr users find and verify
                you. It appears as <Text fontWeight="700">you@{DOMAIN}</Text> in compatible apps.
                Registration is free and powered by Bey Wallet.
              </Text>
            </YStack>
          </YStack>
        )}
