...
import { ThemeModal } from '~/features/settings/components/ThemeModal';
import { CurrencyModal } from '~/features/settings/components/CurrencyModal';
import { NotificationsModal } from '~/features/settings/components/NotificationsModal';
import { BiometricModal } from '~/features/settings/components/BiometricModal';
import { MintModal } from '~/features/settings/components/MintModal';
import { SettingSection } from '~/features/settings/components/SettingSection';
import { DeleteWalletSheet } from '~/features/settings/components/DeleteWalletSheet';
import { SettingSectionConfig } from '~/features/settings/components/types';
import NostrIcon from '~/shared/icons/NostrIcon';
...
import { consolidationService } from '~/services/wallet';
import { proofService } from '~/services/wallet';
...
import { walletFileService } from '~/services/platform/walletFileService';
import Constants from 'expo-constants';
import { useNip05Lookup } from '~/shared/hooks/useNip05Lookup';
import * as Updates from 'expo-updates';
import { useWalletStore } from '~/state/walletStore';
import { InfoSheet, InfoSheetRef } from '~/shared/ui/InfoSheet';
...
export function SettingsScreen() {
    ...
    switch (id) {
      case 'backup':
        const success = await biometricService.authenticateAsync(
          'Authorize to view your secret backup phrase',
        );
        if (success) {
    ...
    try {
      ...
      if (results.length === 0) {
        infoSheetRef.current?.present({
          title: 'No DLEQ Data',
          description:
            'None of your stored proofs have DLEQ data. This is normal for older proofs or mints that do not provide DLEQ.',
          type: 'info',
        });
        return;
      }
  ...
  const handleExportWallet = async () => {
    const authed = await biometricService.authenticateAsync(
      'Authenticate to export your wallet backup',
    );
    if (!authed) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      return;
    ...
    try {
      ...
      const { backupService } = require('~/services/wallet/backupService');
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                'Restore Completed',
                'Your wallet has been restored successfully. Please restart the app.',
                [
                  {
                    text: 'OK',
                    onPress: () => {
      ...
      items: [
        {
          id: 'mint',
          title: 'Fallback Mint',
          value: defaultMintUrl
            ? (() => {
                try {
                  return new URL(defaultMintUrl).hostname;
                } catch {
    ...
    <ScrollView bg="$background" showsVerticalScrollIndicator={false}>
      <YStack flex={1} p="$4" gap="$3" pb="$20">
        {SETTINGS_CONFIG.map((section) => (
          <SettingSection key={section.title} {...section} onItemPress={handleSettingPress} />
        ))}
