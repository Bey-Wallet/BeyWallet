...
import { formatFullLocalTime, formatRelativeTime } from '~/shared/utils/time';
import {
  historyService,
  initService,
  proofService,
...
import { PendingTokenLayout } from '~/shared/ui/PendingTokenLayout';
import { PendingMintInvoiceLayout } from '~/features/history/components/PendingMintInvoiceLayout';
...
import { StatusBadge, BadgeStatus } from '~/shared/ui/StatusBadge';
...
export function TransactionDetailsScreen() {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (repo?.historyRepository) {
          ...
          if (dbEntry) {
            console.log(
              '[TransactionDetails] Loaded entry directly by ID from historyRepository:',
              dbEntry,
            );
      ...
      try {
        ...
        if (repo?.historyRepository && typeof id === 'string') {
          ...
          if (dbEntry) {
            console.log(
              '[TransactionDetails] Found entry by quoteId/id in paginated historyRepository entries:',
              dbEntry,
            );
            return dbEntry as HistoryEntry;
          }
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (repo?.mintQuoteRepository) {
          const quote = await repo.mintQuoteRepository.getMintQuote(
            entry.mintUrl,
            (entry as any).quoteId,
          );
    ...
    const timer = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (
          ...
          entry &&
          (entry.state === 'UNPAID' || entry.state === 'PAID')
        ) {
          (repo.historyRepository as any)
            .updateHistoryMintEntry(entry.mintUrl, (entry as any).quoteId, 'failed')
            .then(() => {
            ...
            .catch((e) =>
              console.warn('[TransactionDetails] Failed to update expired quote in DB:', e),
            );
        } else {
          refetch();
          refetchMintQuote();
  ...
  useEffect(() => {
    ...
    if (entry.token) {
      try {
        ...
        const encoded =
          typeof tokenToEncode === 'string' ? tokenToEncode : encodeToken(tokenToEncode);
  ...
  useEffect(() => {
    ...
    const isPendingMint =
      entry.type === 'mint' &&
      (status.toUpperCase() === 'UNPAID' || status.toUpperCase() === 'PAID');
  ...
  const badgeStatus: BadgeStatus = (() => {
    ...
    if (s === 'success' || s === 'claimed' || s === 'completed' || s === 'paid' || s === 'refunded')
      return 'success';
        ...
        <PendingMintInvoiceLayout
          ...
          onPaid={async () => {
            ...
            try {
              ...
              toast.show('Not Paid Yet', {
                message: 'The invoice has not been paid yet. Please wait or try again.',
              });
            } finally {
      ...
      <>
        ...
        <ScrollView
          ...
          contentContainerStyle={
            { flexGrow: 1, paddingBottom: 120, paddingHorizontal: 16, paddingTop: 16 } as any
          }
        >
          <PendingTokenLayout
            token={token}
    ...
    return (
      <>
        ...
        <ScrollView
          ...
          showsVerticalScrollIndicator={false}
          contentContainerStyle={
            { paddingBottom: 120, paddingHorizontal: 16, paddingTop: 16 } as any
          }
        >
                      ...
                      : entry.type === 'swap' || metadata?.via === 'swap'
                        ...
                        : entry.type === 'send'
                          ...
                          : entry.type === 'receive'
                            ...
                            : entry.type === 'mint'
                              ? 'Ecash added to your wallet.'
                              : entry.type === 'melt'
                                ? metadata?.via === 'onchain'
                                  ? 'On-chain payment was broadcast successfully.'
                                  : 'Lightning invoice was successfully paid.'
