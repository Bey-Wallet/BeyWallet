...
import { onchainNetworkDisplay } from '~/shared/utils/onchain';
      ...
      try {
        ...
        if (isMounted) {
          ...
          try {
            ...
            console.log(
              '[OnchainMintFlow] Saved on-chain quote in local database history:',
              data.quote,
            );
          } catch (dbErr) {
  ...
  const handleCheckPayment = useCallback(
    async (showToast = true) => {
      ...
      try {
        ...
        if (showToast) {
          toast.show('Error checking payment', {
      ...
      <YStack flex={1} justify="space-between" bg="$background">
        <YStack flex={1} justify="center" items="center" gap="$4" p="$4">
          ...
          <Text color="$gray10" fontSize="$4" text="center" px="$4">
            {errorMsg || 'Could not request a deposit address.'}
          </Text>
        </YStack>
        <Button
          theme="accent"
    ...
    <YStack flex={1} bg="$background" justify="space-between">
      ...
      >
        <YStack items="center" gap="$4" mb="$4">
          ...
          >
            ...
            <Button
              ...
              circular
              icon={
                copied ? <Check size={18} color="$green10" /> : <Copy size={18} color="$color" />
              }
              onPress={handleCopy}
            />
          </XStack>
        ...
        <ListTable>
          <ListTableRow
            label="Network"
            value={quoteData ? onchainNetworkDisplay(quoteData.request) : 'Bitcoin'}
            valueColor="$orange10"
          />
          <ListTableRow
