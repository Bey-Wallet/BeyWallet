...
import { useNostrRequestStore } from '~/state/nostrRequestStore';
...
import { walletService, historyService } from '~/services/wallet';
...
import { sendNostrToken } from '~/services/wallet/nostrService';
...
export function RequestEcashStage({
  ...
  React.useEffect(() => {
    if (initialRequestId) {
      ...
      loadPendingRequests().then(() => {
        const req = useNostrRequestStore
          .getState()
          .pendingRequests.find((r) => r.id === initialRequestId);
        if (req) {
  ...
  React.useEffect(() => {
    ...
    const subReceived = DeviceEventEmitter.addListener('nostr:received', (data: any) => {
      ...
      const isMatch =
        (data.requestId && data.requestId === currentRequestId) ||
        (data.amount === amtNum &&
          data.mintUrl?.replace(/\/$/, '') === activeMintUrl?.replace(/\/$/, ''));
        ...
        setTimeout(async () => {
          try {
            ...
            const entry = history.find(
              (e: any) =>
                e.type === 'receive' &&
                Number(e.amount) === amtNum &&
                e.mintUrl.replace(/\/$/, '') === activeMintUrl?.replace(/\/$/, ''),
            );
            if (entry) {
  ...
  const handleCheckStatus = useCallback(async () => {
    ...
    try {
      ...
      const { nostrService } = require('~/services/wallet/nostrService');
                ...
                <>
                  {targetNpub ? (
                    <XStack items="center" gap="$2" justify="center" flexWrap="wrap">
                      ...
                      <XStack bg="$gray4" px="$2.5" py="$1" rounded="$10" items="center" gap="$1.5">
                        <Blockies
                          seed={targetNpub}
                          size={4}
                          scale={2}
        ...
        <YStack flex={1} px="$4" pb="$8" pt="$4" gap="$4">
          ...
          >
            ...
            {targetNpub ? (
              <DetailItem
                label="Sent To"
                value={targetUsername || `${targetNpub.slice(0, 10)}...${targetNpub.slice(-6)}`}
              />
            ) : null}
            {publishedLink ? (
              <DetailItem
          ...
          <YStack gap="$3" mt="auto">
            ...
            <XStack justify="center" gap="$4">
              <Button
                ...
                icon={
                  isChecking ? (
                    ...
                    <RefreshCw size={14} color="$accent9" />
                  )
                }
              ...
              >
                <Text fontSize="$2" color="$accent10" fontWeight="700">
                  Check Status
                </Text>
              </Button>
              <Button
