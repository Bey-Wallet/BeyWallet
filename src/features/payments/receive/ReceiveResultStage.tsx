...
export function ReceiveResultStage({
  ...
  useEffect(() => {
    ...
    if (mint && proofCount > 0) {
      const { mintManager } = require('~/services/wallet');
      mintManager
        .getFeePpk(mint)
        .then((feePpk: number) => {
  ...
  const isOfflineSaved =
    !isSuccess &&
    error &&
    (error.includes('Could not connect to mint') ||
      error.includes('saved to your transaction history') ||
      error.includes('saved in your history'));
    ...
    if (isOfflineSaved) {
      return (
        <YStack flex={1} justify="center" items="center" gap="$4" p="$4" bg="$background">
          ...
          <Text color="$orange10" fontSize="$6" fontWeight="700" text="center">
            Received Offline
          </Text>
      ...
      >
        <YStack gap="$4">
          ...
          <XStack
            ...
            items="center"
            gap="$2"
            bg={isReceiveLater ? '$orange9' : '$green9'}
            px="$4"
            py="$3"
            rounded="$10"
          >
          ...
          <YStack px="$4" py="$2">
            <Text color="$gray10" fontSize="$4" text="center" lineHeight={20}>
              {isReceiveLater
                ? 'You can claim this later from History.'
          ...
          <YStack bg="$gray2" rounded="$6" overflow="hidden">
            ...
            <YGroup separator={<Separator borderColor="$borderColor" opacity={0.5} />}>
              ...
              />
              <DetailItem label="Proofs" value={(decoded?.proofs?.length || 0).toString()} />
              <DetailItem label="Fee" value={`${estimatedFee} sats`} />
              <DetailItem label="Time" value={receiveTime} />
            </YGroup>
          </YStack>
              ...
              <Button
                ...
                icon={
                  isLoading ? (
                    ...
                    <ArrowDownLeft size={20} color="$blue10" />
                  )
                }
              >
                Claim Now
