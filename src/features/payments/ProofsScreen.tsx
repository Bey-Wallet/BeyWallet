...
import type { CoreProof } from '~/services/wallet';
      ...
      <XStack gap="$1" items="center" flex={1} justify="flex-end">
        ...
        {onShare && (
          <Button
            size="$2"
            chromeless
            icon={<Share2 size={14} color="$gray10" />}
            onPress={onShare}
          />
        )}
        {isCopyable && (
...
function StatChip({
  ...
  return (
    <YStack items="center" gap="$0.5" px="$3" py="$2" bg="$gray3" rounded="$4" flex={1}>
      <Text fontSize="$6" fontWeight="900" color={(color as any) ?? '$color'}>
        {value}
      </Text>
...
export default function ProofsModal() {
  ...
  const {
    data: unspentProofs = [],
    isFetching: fetchingUnspent,
    refetch: refetchUnspent,
  } = useQuery({
  ...
  const {
    data: inflightProofs = [],
    isFetching: fetchingInflight,
    refetch: refetchInflight,
  } = useQuery({
  ...
  const filteredProofs = useMemo(() => {
    ...
    return allProofsForTab.filter(
      (p) => p.mintUrl?.replace(/\/$/, '') === mintFilter.replace(/\/$/, ''),
    );
  ...
  const openSendStage = async () => {
    ...
    try {
      ...
      const token = proofService.encodeProofsAsToken(selectedProof.mintUrl, [selectedProof], 'sat');

      // Mark proof as inflight instantly so it can't be double-spent from UI
      await initService
        .getRepo()
        .proofRepository.setProofState(selectedProof.mintUrl, [selectedProof.secret], 'inflight');
  ...
  const handleDeleteProof = () => {
    ...
    if (!canDelete) {
      Alert.alert(
        'Cannot Delete',
        'In-flight proofs cannot be deleted — they may be part of an active send operation.',
      );
      return;
    }
    Alert.alert(
        ...
        <YStack py="$12" items="center" justify="center" gap="$3" opacity={0.5}>
          ...
          <YStack items="center" gap="$1">
            <Text fontWeight="700" color="$color">
              No {meta.label.toLowerCase()} proofs
            </Text>
      ...
      >
        <YStack p="$4" gap="$4">
          ...
          >
            ...
            />
            <DetailItem label="State" value={selectedProof.state} />
          </YStack>
          ...
          {isInflight && (
            <YStack p="$3" bg="$orange2" rounded="$4" gap="$1">
              <XStack gap="$2" items="center">
                ...
                <Text fontWeight="700" color="$orange10" fontSize="$3">
                  In-flight Proof
                </Text>
              </XStack>
              <Text fontSize="$2" color="$orange10">
                This proof is currently locked to a pending send operation. It cannot be exported or
                deleted until the operation completes or is rolled back.
              </Text>
            </YStack>
          )}
  ...
  return (
    <YStack flex={1} bg="$background">
        ...
        <YStack px="$4" pt="$4" gap="$4">
          ...
          <XStack justify="space-between" items="center">
            ...
            <View bg="$gray2" px="$2" py="$1" rounded="$3">
              <Text fontSize="$3" color="$color" fontWeight="800">
          ...
          >
            ...
            <Text fontSize="$2" color="$gray9" lineHeight="$3">
              Proofs are cryptographic bearer tokens representing your ecash balance. Each has a
              denomination, a secret, and a blind signature from the mint. Unspent proofs =
              spendable funds.
            </Text>
          </YStack>
        </YStack>
      ...
      <AppBottomSheet ref={proofActionSheetRef} snapPoints={['90%']}>
