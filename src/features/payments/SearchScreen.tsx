...
import { MintIcon } from '~/features/explore/components/MintIcon';
...
export default function UniversalSearchScreen() {
  ...
  const doSearch = useCallback(
    (query: string) => {
      ...
      mints.forEach((mint) => {
        if (
          mint.mintUrl.toLowerCase().includes(lowerQuery) ||
          (mint.nickname && mint.nickname.toLowerCase().includes(lowerQuery))
        ) {
          mintResults.push({
          ...
          {item.subtitle && (
            <Text fontSize="$3" color="$gray10" numberOfLines={1}>
              {item.subtitle.length > 30
                ? `${item.subtitle.slice(0, 6)}...${item.subtitle.slice(-6)}`
                : item.subtitle}
            </Text>
          )}
        </YStack>
        ...
        <XStack flex={1} bg="$gray3" rounded="$5" px="$3" height={50} items="center" gap="$2">
          ...
          {isSearching ? (
            ...
            <Button
              size="$2"
              chromeless
              icon={<Scan strokeWidth={3} size={20} color="$gray10" />}
              onPress={handleScan}
            />
          )}
        </XStack>
      ...
      <YStack flex={1} width="100%">
        {search.trim() !== '' && results.length === 0 && !isSearching ? (
          ...
          <YStack flex={1} items="center" justify="center" gap="$3" px="$4">
            ...
            <Text fontSize="$5" fontWeight="600" color="$gray8">
              Search anything
            </Text>
            <Text color="$gray8" textAlign="center">
