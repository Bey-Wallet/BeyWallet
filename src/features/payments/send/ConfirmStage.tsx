...
import { SendMode } from '~/shared/ui/SendMethodSelector';
...
export function ConfirmStage({
  ...
  const displayRecipientText = useMemo(() => {
    if (sendMode === 'nostr') {
      return (
        nostrRecipientUsername ||
        (nostrRecipientNpub ? `${nostrRecipientNpub.slice(0, 10)}...` : '')
      );
    }
          ...
          <YStack bg="$gray2" rounded="$6" overflow="hidden" mb="$3">
            ...
            <YGroup separator={<Separator borderColor="$borderColor" opacity={0.5} />}>
              ...
              <DetailItem
                ...
                value={
                  estimatedFee > 0
                    ? `~${currencyService.formatSats(estimatedFee)}`
                    : `${currencyService.formatSats(0)} (Free)`
                }
              ...
              <XStack justify="space-between" items="center" py="$3" px="$4">
                ...
                <XStack gap="$2" items="center">
                  <Text
                    fontSize="$2"
                    color={expiryEnabled ? '$green11' : '$gray10'}
                    fontWeight="700"
                  >
