...
export function ResultStage({
  ...
  const primaryAmountLabel =
    primaryCurrency === 'SATS' ? currencyService.formatSats(Number(amount || 0)) : fiatValue;
  const secondaryAmountLabel =
    primaryCurrency === 'SATS' ? fiatValue : currencyService.formatSats(Number(amount || 0));
    ...
    return (
      <YStack flex={1} bg="$background">
        ...
        <YStack flex={1} justify="center" items="center" gap="$4" p="$4">
          ...
          <Button theme="accent" size="$5" width="100%" onPress={onClose} mt="$4">
            Go Back
          </Button>
        </YStack>
      </YStack>
      ...
      >
        {token ? (
          ...
          <YStack gap="$4">
            ...
            {/* Details List */}
            <YStack bg="$gray2" rounded="$6" overflow="hidden" mb="$6">
              <View p="$3" px="$4">
              ...
              <YGroup separator={<Separator borderColor="$borderColor" opacity={0.5} />}>
                ...
                <DetailItem label="Time" value={sendTime} />
              </YGroup>
            </YStack>
          </YStack>
        )}
