...
import { AmountStage } from '~/features/payments/send/AmountStage';
import { ConfirmStage } from '~/features/payments/send/ConfirmStage';
import { P2PKAmountStage } from '~/features/payments/send/P2PKAmountStage';
...
import { seedService } from '~/services/platform/seedService';
...
import { currencyService, SUPPORTED_CURRENCIES } from '~/services/wallet/currencyService';
import {
  Building2,
  ShieldCheck,
  Zap,
...
import { eventService, proofService } from '~/services/wallet';
import SendMethodSelector, { SendMode } from '~/shared/ui/SendMethodSelector';
import { PaymentRequest, PaymentRequestTransportType } from '@cashu/cashu-ts';
import Blockies from '~/shared/ui/Blockies';
import {
  OfflineOptimizationSheet,
  type OfflineOptimizationSheetRef,
} from '~/shared/ui/OfflineOptimizationSheet';
...
export function SendModalScreen() {
  ...
  const handleOfflineOptimizationConfirm = useCallback((optimizedAmount: number, proofs: any[]) => {
    console.log(
      '[SendModalScreen] Offline optimization confirmed amount:',
      optimizedAmount,
      'proofs:',
      proofs.length,
    );
    ...
    const checkOfflineMode = async () => {
      try {
        ...
        if (offline && (params.mode === 'nostr' || params.mode === 'scan')) {
          Alert.alert(
            'You are offline',
            `${params.mode === 'nostr' ? 'Nostr Send' : 'Scan & Pay'} is not available offline. Switched to Standard Mode.`,
          );
  ...
  React.useEffect(() => {
    ...
    const handleSuccess = async (source: string) => {
      ...
      if (source === 'event') {
        try {
          ...
          if (!allSpent) {
            console.warn(
              '[SendModalScreen] ⚠️ Event claimed but proofs still UNSPENT. Ignoring premature event.',
            );
            return;
          }
        } catch (err) {
    ...
    const pollOnce = async () => {
      ...
      try {
        ...
        console.log(
          `[SendModalScreen] 🔍 Poll [${operationId}]: ${spentCount}/${states.length} SPENT`,
        );
    ...
    try {
      ...
      if (isOffline) {
        ...
        if (!selected || selected.length === 0) {
          ...
          const { findExactSubset } = require('~/shared/utils/offlineSendUtils');
        ...
        const { buildEcashNostrEvent } = require('~/shared/utils/ecashSharing');
        const { event } = buildEcashNostrEvent(result.token, secretKeyHex);
        const { SimplePool } = require('nostr-tools');
        const { RELAYS } = require('~/services/wallet/nostrService');
        ...
        setEncodedToken(`${websiteUrl}#${secretKeyHex}`);
        setOperationId(result.id);
      } else {
        result = await walletService.send(activeMintUrl, amountSats);
        rawTokenRef.current = result.token;
      ...
      refreshBalance();
      console.log(
        '[SendModalScreen] Send successful. OpId:',
        result.id,
        'Token length:',
        (result.encoded || result.token || '').length,
      );
  ...
  const handleNostrSend = useCallback(async () => {
    ...
    try {
        ...
        .tagHistoryVia(
          ...
          'nostr',
          {
            ...
            ...(nostrRecipientUsername
              ? { nostrUsername: nostrRecipientUsername.replace('@bey.cash', '') }
              : {}),
            ...(computedExpiry
              ? {
                  expiresAt: computedExpiry,
      ...
      console.log(
        `[SendModal] ✅ Nostr send complete: ${amountSats} sats to ${nostrRecipientNpub.slice(0, 10)}…`,
      );
    } catch (err: any) {
  ...
  const handleScanContinue = (forcedInput?: string) => {
    ...
    const isPaymentRequest =
      input.toLowerCase().startsWith('creqa') || input.toLowerCase().startsWith('creqb');
  ...
  return (
    <YStack flex={1} bg="$background" px="$4">
      <Stack.Screen
        options={{
          headerTitle: () => <H4 fontWeight="bold">{headerTitle}</H4>,
      ...
      {step === 'amount' && (
        <YStack flex={1}>
          {(() => {
            console.log(
              '[SendModalScreen] Rendering step: amount, sendMode:',
              sendMode,
              'isOffline:',
              isOffline,
            );
