...
import NFCFill2 from '~/shared/icons/NFC-fill-2';
import { ProcessingSheet, ProcessingStatus } from '~/shared/ui/ProcessingSheet';
...
import { nfcService } from '~/services/platform/nfcService';
...
import { proofService } from '~/services/wallet/proofService';
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (states && states.length > 0) {
          const isSpent = states.some(
            (s: any) => s.state === 'SPENT' || String(s.state).toUpperCase() === 'SPENT',
          );
                ...
                <Text color="$color" fontSize="$5" fontWeight="700" textAlign="center">
                  {transferState === 'transferred'
                    ...
                    : transferState === 'transferring'
                      ...
                      : hceActive
                        ? 'Ready to Tap'
                        : hceError || 'NFC Broadcast Inactive'}
                </Text>
                <Text color="$gray9" fontSize="$3" textAlign="center" maxWidth={280}>
                  {transferState === 'transferred'
                    ? 'Waiting for receiver wallet to claim...'
                    : transferState === 'transferring'
                      ? 'Hold phones steady back-to-back'
                      : hceActive
                        ? 'Hold your phone back-to-back near receiver device'
            ...
            <XStack gap="$2" bg="$gray2" p="$1" rounded="$5">
              <Button
                ...
                icon={
                  <RadioReceiver
                    size={18}
                    color={mode === 'hce' ? theme.background.val : theme.color.val}
                  />
                }
              >
                Phone to Phone
              ...
              <Button
                ...
                icon={
                  <Tag
                    size={18}
                    color={mode === 'write' ? theme.background.val : theme.color.val}
                  />
                }
              >
                Write to Tag
