...
import { AmountStage } from '~/features/payments/swap/AmountStage';
import { ResultStage } from '~/features/payments/swap/ResultStage';
...
export default function SwapScreen() {
  ...
  const firstFunded = React.useMemo(() => {
    return (
      activeMintUrl ||
      mints.find((m) => (balances[m.mintUrl] || 0) > 0)?.mintUrl ||
      mints[0]?.mintUrl
    );
  ...
  const handleSwap = useCallback(async () => {
    ...
    try {
      if (sourceMintUrl === targetMintUrl) {
        ...
        console.log(
          `[Swap] Executing NUT-19 cross-mint swap (${amountNum} sats) from ${sourceMintUrl} to ${targetMintUrl}`,
        );
        ...
        if (repo?.historyRepository) {
          await (repo.historyRepository as any).addHistoryEntry({
            ...
            metadata: {
              ...
              targetMintUrl,
              sourceMintName:
                sourceMint?.name || sourceMintUrl.replace(/^https?:\/\//, '').split('/')[0],
              targetMintName:
                targetMint?.name || targetMintUrl.replace(/^https?:\/\//, '').split('/')[0],
            },
          });
        }
      } catch (histErr) {
                ...
                <YGroup separator={<Separator borderColor="$borderColor" opacity={0.5} />}>
                  <DetailItem
                    ...
                    value={
                      sourceMint?.nickname ||
                      sourceMint?.name ||
                      sourceMintUrl.replace(/^https?:\/\//, '').split('/')[0]
                    }
                    icon={
                  ...
                  <DetailItem
                    ...
                    value={
                      targetMint?.nickname ||
                      targetMint?.name ||
                      targetMintUrl.replace(/^https?:\/\//, '').split('/')[0]
                    }
                    icon={
                      <Avatar rounded="$3" size="$1.5">
