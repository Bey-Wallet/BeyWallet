        ...
        try {
            ...
            if (reqMatch) {
                ...
                if (pr.mints && pr.mints.length > 0) {
                    const match = mints.find(m => pr.mints!.some(pm => normalize(pm) === normalize(m.mintUrl)));
                    if (match) {
                        targetMint = match.mintUrl;
                    } else {
                        throw new Error(`Payment requires one of: ${pr.mints.map(m => m.replace(/^https?:\/\//, '')).join(', ')}`);
                    }
                }
                ...
                if (amountSats > currentBal) {
                    throw new Error(`Insufficient balance. Need ${amountSats} sats, have ${currentBal} sats.`);
                }
            ...
            if (lnMatch) {
                ...
                const currentBalance = activeMintUrl ? (balances[activeMintUrl] ?? 0) : (balances[mintToUse] ?? 0);
                ...
                if (totalCost > currentBalance) {
                    throw new Error(`Insufficient balance. Need ${totalCost} sats (${quote.amount} + ${quote.fee_reserve} fee), have ${currentBalance} sats.`);
                }
        ...
        <Flex fill bg="$background" pb={insets.bottom || 16}>
            ...
            <YStack flex={1} bg="$background" px="$4" justify="space-between">
                    ...
                    {!isNfcSupported ? (
                        ...
                        <YStack items="center" gap="$3">
                            ...
                            <Text color="$green10" fontSize="$6" fontWeight="800">
                                {receivedAmount !== null ? `-${currencyService.formatSats(receivedAmount)}` : 'Payment Complete!'}
                            </Text>
                        ...
                        <YStack items="center" gap="$3">
                            ...
                            <Text color="$gray9" fontSize="$3" textAlign="center">
                                {receivedAmount ? `Creating ${receivedAmount} sats token & transmitting over NFC...` : 'Settling invoice with merchant...'}
                            </Text>
                        </YStack>
                ...
                <YStack gap="$2">
                    ...
                    >
                        {!isNfcEnabled ? "Turn on NFC" : receiveState === 'error' ? "Retry Tap" : "Tap to Receive / Pay"}
                    </Button>
                </YStack>
            </YStack>
        </Flex>
