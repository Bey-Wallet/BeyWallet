...
import { OfflineDenominationSelector } from '~/shared/ui/OfflineDenominationSelector';
import { findClosestSubsetOptions, SubsetOption } from '~/shared/utils/offlineSendUtils';
...
export const OfflineOptimizationSheet = forwardRef<
  OfflineOptimizationSheetRef,
  OfflineOptimizationSheetProps
>(({ targetAmount, activeMintUrl, onConfirm }, ref) => {
  ...
  const [selectedOptionType, setSelectedOptionType] = useState<'lower' | 'exact' | 'higher' | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const loadProofs = async () => {
      try {
        const { initService } = require('~/services/wallet');
        ...
        {viewMode === 'closest' ? (
          <YStack gap="$4">
            <Text color="$gray10" fontSize="$3" textAlign="center" px="$2">
              We are offline. Choose a possible sat amount close to your target or select coins
              manually.
            </Text>
                  ...
                  <YStack gap="$1.5">
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$3"
                        fontWeight="600"
                        color={selectedOptionType === 'lower' ? 'white' : '$gray10'}
                      >
                        Send Less
                      </Text>
                      {options.recommended === 'lower' && (
                        <Text
                          ...
                          fontWeight="800"
                          bg={
                            selectedOptionType === 'lower'
                              ? 'rgba(255, 255, 255, 0.25)'
                              : '$accent5'
                          }
                          color={selectedOptionType === 'lower' ? 'white' : '$accent10'}
                          px="$2"
                          py="$0.5"
                          rounded="$2"
                        >
                    ...
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$5"
                        fontWeight="800"
                        color={selectedOptionType === 'lower' ? 'white' : '$color'}
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(targetAmount)
                          : `₿${targetAmount}`}
                      </Text>
                      <ArrowRight
                        size={14}
                      ...
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(options.lower.amount)
                          : `₿${options.lower.amount} sats`}
                      </Text>
                    </XStack>
                    <Text
                      fontSize="$2"
                      color={
                        selectedOptionType === 'lower' ? 'rgba(255, 255, 255, 0.7)' : '$gray10'
                      }
                    >
                      {primaryCurrency === 'FIAT'
                        ? `₿${options.lower.amount} sats`
                        : getFiatValue(options.lower.amount)}{' '}
                      &bull; -{targetAmount - options.lower.amount} sats difference
                    </Text>
                  </YStack>
                  <View
                    width={24}
                  ...
                  <YStack gap="$1.5">
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$3"
                        fontWeight="600"
                        color={selectedOptionType === 'exact' ? 'white' : '$gray10'}
                      >
                        Exact Match
                      </Text>
                      <Text
                        ...
                        fontWeight="800"
                        bg={
                          selectedOptionType === 'exact' ? 'rgba(255, 255, 255, 0.25)' : '$green5'
                        }
                        color={selectedOptionType === 'exact' ? 'white' : '$green10'}
                        px="$2"
                        py="$0.5"
                        rounded="$2"
                      >
                    ...
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$5"
                        fontWeight="800"
                        color={selectedOptionType === 'exact' ? 'white' : '$color'}
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(targetAmount)
                          : `₿${targetAmount}`}
                      ...
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(options.exact.amount)
                          : `₿${options.exact.amount} sats`}
                      </Text>
                    </XStack>
                    <Text
                      fontSize="$2"
                      color={
                        selectedOptionType === 'exact' ? 'rgba(255, 255, 255, 0.7)' : '$gray10'
                      }
                    >
                      {primaryCurrency === 'FIAT'
                        ? `₿${options.exact.amount} sats`
                        : getFiatValue(options.exact.amount)}{' '}
                      &bull; Exact subset sum available
                    </Text>
                  </YStack>
                  <View
                    width={24}
                  ...
                  <YStack gap="$1.5">
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$3"
                        fontWeight="600"
                        color={selectedOptionType === 'higher' ? 'white' : '$gray10'}
                      >
                        Send More
                      </Text>
                      {options.recommended === 'higher' && (
                        <Text
                          ...
                          fontWeight="800"
                          bg={
                            selectedOptionType === 'higher'
                              ? 'rgba(255, 255, 255, 0.25)'
                              : '$accent5'
                          }
                          color={selectedOptionType === 'higher' ? 'white' : '$accent10'}
                          px="$2"
                          py="$0.5"
                          rounded="$2"
                        >
                    ...
                    <XStack items="center" gap="$2">
                      <Text
                        fontSize="$5"
                        fontWeight="800"
                        color={selectedOptionType === 'higher' ? 'white' : '$color'}
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(targetAmount)
                          : `₿${targetAmount}`}
                      ...
                      >
                        {primaryCurrency === 'FIAT'
                          ? getFiatValue(options.higher.amount)
                          : `₿${options.higher.amount} sats`}
                      </Text>
                    </XStack>
                    <Text
                      fontSize="$2"
                      color={
                        selectedOptionType === 'higher' ? 'rgba(255, 255, 255, 0.7)' : '$gray10'
                      }
                    >
                      {primaryCurrency === 'FIAT'
                        ? `₿${options.higher.amount} sats`
                        : getFiatValue(options.higher.amount)}{' '}
                      &bull; +{options.higher.amount - targetAmount} sats difference
                    </Text>
                  </YStack>
                  <View
                    width={24}
