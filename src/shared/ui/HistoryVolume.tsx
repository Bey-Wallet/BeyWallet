...
export default function HistoryVolume() {
  ...
  const handleExecuteAction = async () => {
    ...
    try {
      ...
      if (actionType === 'spent_proofs') {
        ...
        toast.show('Cleaned spent proofs', {
          message: `Successfully deleted ${deletedCount} spent proofs.`,
        });
      } else if (actionType === 'dead_invoices') {
        // Delete dead (expired unpaid) quotes
        const nowSeconds = Math.floor(Date.now() / 1000);
        const mintRes = await repo.db.run(
          `DELETE FROM coco_cashu_mint_quotes WHERE state = 'UNPAID' AND expiry < ?`,
          [nowSeconds],
        );
        const meltRes = await repo.db.run(
          `DELETE FROM coco_cashu_melt_quotes WHERE state = 'UNPAID' AND expiry < ?`,
          [nowSeconds],
        );
        const totalDeleted = (mintRes.changes || 0) + (meltRes.changes || 0);
        toast.show('Cleaned dead invoices', {
          message: `Successfully deleted ${totalDeleted} expired quotes.`,
        });
      }
      ...
      toast.show('Cleanup failed', {
        message: e.message || 'An error occurred during database cleanup.',
      });
    } finally {
  ...
  const trigger = (
    <XStack items="center" gap="$1.5" justify="flex-end" pressStyle={{ opacity: 0.8 }}>
      ...
      <AppBottomSheet ref={confirmSheetRef} snapPoints={['38%']}>
        <YStack p="$4" gap="$4">
          <XStack gap="$2" items="center" alignSelf="center">
            ...
            <Text fontSize="$6" fontWeight="800" color="$red10">
              Confirm Delete
            </Text>
          </XStack>
