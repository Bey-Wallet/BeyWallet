...
import { walletService, mintManager, historyService } from '~/services/wallet';
    ...
    const subscription = DeviceEventEmitter.addListener(
      ...
      async (data: {
        ...
        if (existing && (existing.status === 'claimed' || existing.status === 'claiming')) {
          console.log(
            `[NostrClaimSheet] Skipping already ${existing.status} event ${data.eventId.slice(0, 8)}`,
          );
          return;
        }
        ...
        try {
          ...
          console.warn(
            '[NostrClaimSheet] Failed to load pending requests on incoming event:',
            loadErr,
          );
        }
        ...
        const matchingRequest = pending.find(
          (r: any) =>
            (data.requestId && r.id === data.requestId) ||
            (Number(r.amount) === Number(data.amount) &&
              r.mintUrl.replace(/\/$/, '') === data.mintUrl.replace(/\/$/, '') &&
              r.state === 'pending'),
        );
      ...
      if (received) {
        ...
        try {
          ...
          const match = pending.find(
            (r) =>
              r.mintUrl.replace(/\/$/, '') === activeItem.mintUrl.replace(/\/$/, '') &&
        ...
        try {
          ...
          historyService
            .tagHistoryVia(activeItem.mintUrl, 'receive', 'nostr', {
              nostrPubkey: senderNpub,
              nostrUsername: activeItem.senderUsername
                ? activeItem.senderUsername.replace('@bey.cash', '')
                : undefined,
            })
        ...
        <AppBottomSheet ref={sheetRef} onClose={handleDismiss} enablePanDownToClose={true}>
          <YStack p="$4" gap="$3" items="center" width="100%">
            ...
            <YStack gap="$2" py="$4" items="center" justify="center">
              <Text fontSize={52} fontWeight="700" color="$color1" lineHeight={54}>
            ...
            <YStack bg="$gray2" rounded="$5" overflow="hidden" width="100%" mb="$4">
              ...
              <YGroup separator={<Separator borderColor="$borderColor" opacity={0.5} />}>
                ...
                />
                <DetailRow
                  icon={<ShieldCheck size={14} color="$gray10" />}
                  label="Type"
                  value="Nostr DM"
                />
                <DetailRow
