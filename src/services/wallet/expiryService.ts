...
import { cleanToken, decodeToken, encodeToken } from '~/services/wallet/tokenUtils';
...
export const expiryService = {
      ...
      for (const row of rows) {
        try {
          ...
          if (Date.now() > Number(metadata.expiresAt)) {
            console.log(
              `[ExpiryService] Found expired pending token: id=${row.id}, opId=${row.operationId}`,
            );
            ...
            if (allSpent) {
              ...
              console.log(
                `[ExpiryService] Token is unspent. Refunding/Sweeping token back to wallet...`,
              );
              try {
                ...
                console.error(
                  `[ExpiryService] Sweep failed for row ${row.id}:`,
   ...
   */
  getExpiryBySecrets: async (
    secrets: string[],
  ): Promise<{ expiresAt: number; expiryHours: number } | null> => {
    try {
  ...
  startSweeper: (intervalMs = 60000): void => {
    ...
    expiryService
      .checkAndSweepExpiredTokens()
      .catch((e) => console.error('[ExpiryService] Initial sweep error:', e));
    scheduleNext();
