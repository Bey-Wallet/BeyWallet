...
import { cleanToken, encodeToken, decodeToken } from '~/services/wallet/tokenUtils';
...
async function handleOfflineReceive(token: string, mintUrl: string | null, error: any) {
  try {
    ...
    if (!knownMints.includes(finalMintUrl.toLowerCase())) {
      try {
        const { mintManager } = require('~/services/wallet/mintManager');
        ...
        console.warn(
          '[WalletService] Offline Receive: failed to auto-add mint in background:',
          addErr,
        );
      }
    }
    ...
    const exists = historyList.find((item: any) => {
      const itemToken =
        typeof item.tokenJson === 'string' ? item.tokenJson : JSON.stringify(item.token);
      return itemToken && (itemToken.includes(token) || token.includes(itemToken));
    });

    if (exists) {
      console.log(
        '[WalletService] Offline Receive: Token already exists in history as ID:',
        exists.id,
      );
      throw new Error(
        `Could not connect to mint. This token is already saved in your history (ID: ${exists.id}) to claim later.`,
      );
    }
    ...
    throw new Error(
      'Could not connect to mint. The token has been saved to your transaction history so you can claim it later when you are online.',
    );
  } catch (saveErr: any) {
...
export const walletService = {
  ...
  sendP2PK: async (
    ...
    if (lockPubkey.startsWith('npub1') || lockPubkey.startsWith('nprofile1')) {
      try {
        ...
        console.log(
          `[WalletService] Converted Nostr pubkey to compressed SEC1: ${lockPubkey.substring(0, 10)}…`,
        );
      } catch (decodeErr: any) {
      ...
      console.log(
        `[WalletService] Added 02 prefix to raw hex pubkey: ${lockPubkey.substring(0, 10)}…`,
      );
    }
    ...
    try {
      ...
      try {
        // Attempt 1
        txn = await wallet.prepareSwapToSend(
          amount,
          availableProofs,
          { includeFees: true },
          customConfig as any,
        ...
        if (firstErr?.message?.includes('already spent') || firstErr?.message?.includes('11001')) {
          console.log(
            `[WalletService] Caught "already spent" state mismatch. Auto-healing local database...`,
          );
      ...
      console.error(
        '[WalletService] ops.send Full Error:',
        JSON.stringify(opsErr, Object.getOwnPropertyNames(opsErr)),
      );
      throw opsErr;
    }
    ...
    const encoded = encodeToken(token);
    console.log(
      `[WalletService] ✅ P2PK Send execution complete! Token locked to: ${receiverPubkey}`,
    );
  ...
  sendOffline: async (
    ...
    console.log(
      `[WalletService] Creating offline send of ${amount} sats from ${mintUrl} using ${selectedProofs.length} proofs`,
    );
  ...
  receive: async (token: string): Promise<void> => {
    ...
    try {
      ...
      if (msg.includes("Couldn't map") || msg.includes('short keyset')) {
        console.warn(
          '[WalletService] Short keyset ID mapping failed — clearing wallet cache and retrying...',
        );
        try {
          if (typeof unsafeManager.walletService?.clearCache === 'function') {
            unsafeManager.walletService.clearCache(mintUrl ?? '');
      ...
      if (msg.includes('could not be verified') || msg.includes('outputs')) {
        throw new Error(
          'Token proofs could not be verified. The token may have already been redeemed.',
        );
      }
  ...
  restore: async (mintUrl: string): Promise<void> => {
    ...
    if (m.wallet && !(m.wallet as any)._restoreOverridden) {
      ...
      m.wallet.restore = async function (targetMintUrl: string) {
        ...
        this.logger?.debug(
          `[WalletService] Filtered keysets for ${targetMintUrl}: ${satKeysets.length} of ${mint.keysets.length} are sat/BTC`,
        );
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[WalletService] Pre-warm step failed (SDK restore may still work):`,
          preWarmErr,
        );
      }
      ...
      if (
        err?.message?.includes('restore some keysets') ||
        err?.message?.includes('Keyset has no keys')
      ) {
        console.warn(
          `[WalletService] ⚠️ Partial restore for ${mintUrl} (some old keysets missing keys, saved proofs are intact):`,
          err?.message,
        );
      } else {
  ...
  restoreInflightProofs: async (mintUrl: string): Promise<number> => {
    ...
    try {
      ...
      console.log(
        `[WalletService] ✅ Restored ${secrets.length} stuck inflight proofs on ${mintUrl}`,
      );
