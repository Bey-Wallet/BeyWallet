...
function hashToCurve(secret: Uint8Array): ReturnType<typeof secp256k1.ProjectivePoint.fromHex> {
  ...
  while (counter < 1000) {
    const msgToHash = new Uint8Array([
      ...new TextEncoder().encode('Secp256k1_HashToCurve_Cashu_'),
      ...secret,
      counter & 0xff,
    ]);
...
export const proofService = {
  ...
  checkProofStates: async (tokenString: string) => {
    ...
    try {
      console.log(
        `[ProofService] Checking state for ${decoded.mint} with ${decoded.proofs.length} proofs`,
      );
      ...
      try {
        ...
        if (!keypairForAmount) {
          results.push({
            ...
            amount,
            valid: false,
            reason: `No keypair for denomination ${amount}`,
          });
          continue;
        }

        // Handle both { publicKey: string } and raw string formats
        const mintPubkeyHex: string =
          typeof keypairForAmount === 'string'
            ? keypairForAmount
        ...
        if (!mintPubkeyHex || typeof mintPubkeyHex !== 'string') {
          results.push({
            ...
            amount,
            valid: false,
            reason: 'Could not extract mint public key from keypair',
          });
          continue;
        }
        ...
        results.push({
          ...
          amount,
          valid: false,
          reason: err.message || 'Unexpected error during DLEQ verification',
        });
      }
    }
